...
//	final Logger L = Logger.getLogger(UserAspect.class);
	...
	public void beforeGet(JoinPoint jp) {
		...
		try { // Check if the request is from someone who's logged in
			...
			if (u.getAccessLevel() < 1 && u.getUserId() != pathVarInt && !u.getUsername().equals(pathVar)) {
				// TODO LOG EXCEPTION
	...
	public void beforeUpdate(JoinPoint jp) {
		...
		try { // Check if the request is from someone who's logged in
			u = (User) session.getAttribute("user");
			if (u.getAccessLevel() < 1 && u.getUserId() != updated.getUserId() && !u.getUsername().equals(updated.getUsername())) {
				// TODO LOG EXCEPTION
